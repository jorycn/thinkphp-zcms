	...
	<div class="g-wrap">
					...
					<?php if(!empty($_list)): if(is_array($_list)): $i = 0; $__LIST__ = $_list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><tr>
						...
						<td>
							<?php if(empty($vo["uninstall"])): $class = get_addon_class($vo['name']); if(!class_exists($class)){ $has_config = 0; }else{ $addon = new $class(); $has_config = count($addon->getConfig()); } ?>
								...
								<?php endif ?>
							<?php if ($vo['status'] >=0): ?>
								<?php if(($vo["status"]) == "0"): ?><a class="J_ajax_get" href="<?php echo U('enable',array('id'=>$vo['id']));?>">启用</a>
								<?php else: ?>
									<a class="J_ajax_get" href="<?php echo U('disable',array('id'=>$vo['id']));?>">禁用</a><?php endif; ?>
							<?php endif ?>
								
									<a class="J_ajax_get" href="<?php echo U('uninstall?id='.$vo['id']);?>">卸载</a>
								
							<?php else: ?>
								<a class="J_ajax_get" href="<?php echo U('install?addon_name='.$vo['name']);?>">安装</a><?php endif; ?>
						</td>
		...
		<div class="btn_wrap">
            <div class="btn_wrap_pd">
            	...
            	<a href="<?php echo U('create');?>" class="btn btn_submit">快速创建</a>
            </div>
        </div>
		<!-- 分页 -->
	    <div class="page">
