	...
	<div class="g-wrap">
			    ...
			    <tbody>
					<?php if(is_array($_list)): $i = 0; $__LIST__ = $_list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><tr>
						...
						<td><a href="<?php echo U('User/editAction?id='.$vo['id']);?>">编辑</a>
							<a href="<?php echo U('User/setStatus?Model=action&ids='.$vo['id'].'&status='.abs(1-$vo['status']));?>" class="J_ajax_get"><?php echo (show_status_op($vo["status"])); ?></a>
							<a href="<?php echo U('User/setStatus?Model=action&status=-1&ids='.$vo['id']);?>" class="confirm J_ajax_get">删除</a>
			                </td>
		...
		<div class="btn_wrap">
			<div class="btn_wrap_pd">
		        ...
		        <button class="btn J_ajax_post" target-form="ids" url="<?php echo u('setstatus',array('status'=>1));?>" >启 用</button>
		        <button class="btn J_ajax_post" target-form="ids" url="<?php echo u('setstatus',array('status'=>0));?>">禁 用</button>
		        <button class="btn J_ajax_post confirm" target-form="ids" url="<?php echo U('setStatus',array('status'=>-1));?>">删 除</button>
		    </div>
		</div>
		<!-- 分页 -->
	    <div class="page">
